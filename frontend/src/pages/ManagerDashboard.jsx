    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                padding: '14px 16px', borderBottom: '1px solid rgba(0,212,255,0.1)',
            ...
            {/* KPI cards — net profit hidden for manager */}
            ...
            <div style={{ padding: '16px' }}>
                ...
                {tab === 'fleet' && (
                    loading ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {drivers.length === 0 ? (
                                <p style={{ textAlign: 'center', color: 'rgba(226,232,240,0.3)', padding: 32 }}>No drivers yet</p>
                            ) : drivers.map(d => (
                                <div key={d.id} style={{
                                    background: 'rgba(4,20,40,0.9)', border: '1px solid rgba(0,212,255,0.1)',
                                    borderRadius: 12, padding: '16px',
                                }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                                        <div>
                                            <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: '#fff', letterSpacing: '0.04em' }}>{d.name}</p>
                                            <p style={{ fontSize: 11, color: 'rgba(226,232,240,0.4)', marginTop: 2 }}>{d.plate} · ID: {d.id}</p>
                                        </div>
                                        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 12, fontWeight: 600, color: '#F5A623', background: 'rgba(245,166,35,0.08)', border: '1px solid rgba(245,166,35,0.2)', padding: '3px 8px', borderRadius: 4 }}>{d.plate}</span>
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                                        {[
                                            { label: 'Trips', value: d.trips, color: '#fff' },
                                            { label: 'Fuel ETB', value: fmt(d.fuel), color: '#00D4FF' },
                ...
                {tab === 'requests' && (
                    <div>
                        <h3 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: '#fff', letterSpacing: '0.06em', marginBottom: 16 }}>DRIVER REQUESTS</h3>
                        {requests.length === 0 ? (
                ...
                {tab === 'trips' && (
                    <div>
                        <h3 style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: '#fff', letterSpacing: '0.06em', marginBottom: 16 }}>TRIP APPROVALS</h3>
                        {tripReqs.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: 48, color: 'rgba(226,232,240,0.25)', fontFamily: "'Rajdhani',sans-serif", fontSize: 14, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>No trip requests yet</div>
                        ) : tripReqs.map(r => (
                            <div key={r._id} style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                    <div>
                                        <p style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, color: '#fff', letterSpacing: '0.04em' }}>🚛 {r.driverName}</p>
                                        <p style={{ fontSize: 12, color: 'rgba(226,232,240,0.5)', marginTop: 2 }}>{r.plate}</p>
                                        <p style={{ fontSize: 11, color: 'rgba(226,232,240,0.3)', marginTop: 2 }}>{new Date(r.createdAt).toLocaleString()}</p>
                                    </div>
                                    <span style={{
                                ...
                                {r.status === 'pending' && (
                                    <div style={{ display: 'flex', gap: 8 }}>
                                        <button onClick={() => handleApproveTrip(r._id)} style={{ flex: 1, padding: '10px', background: 'linear-gradient(135deg, #00C853, #00E676)', color: '#020B18', fontFamily: "'Bebas Neue',sans-serif", fontSize: 16, letterSpacing: '0.06em', borderRadius: 8, border: 'none', cursor: 'pointer' }}>✓ APPROVE TRIP</button>
                                        <button onClick={() => handleDenyTrip(r._id)} style={{ flex: 1, padding: '10px', background: 'rgba(255,71,87,0.1)', border: '1px solid rgba(255,71,87,0.3)', color: '#FF4757', fontFamily: "'Bebas Neue',sans-serif", fontSize: 16, letterSpacing: '0.06em', borderRadius: 8, cursor: 'pointer' }}>✗ DENY</button>
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    <div style={{ maxWidth: 500, margin: '0 auto' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            {[
                                ...
                                <div key={f.key}>
                                    <label style={{ display: 'block', fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(0,212,255,0.55)', marginBottom: 6 }}>{f.label}</label>
                            ...
                            <button onClick={handleAddDriver} disabled={addLoading} style={{
                                ...
                                cursor: addLoading ? 'not-allowed' : 'pointer', boxShadow: '0 6px 25px rgba(0,212,255,0.25)',
                            }}>
                                ...
                                return (
                                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <p style={{ fontSize: 10, color: 'rgba(226,232,240,0.3)', marginTop: 2 }}>{new Date(a.logged_at).toLocaleString()}</p>
                                        </div>
